...
For example, if the tile was an empty tile with no mines surrounding it, revealTile would recurse over the tiles
surrounding it, effectively revealing all tiles that are adjacent to it that are either empty or have a number of
mines surrounding it. However, it is limited whenever the tile has a number of mines surrounding it, so it does 
not reveal too much of the board. Essentially, the function stops recursing when it recognizes that the tile it 
is on has at least one mine adjacent to it.
